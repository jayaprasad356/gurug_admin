        ...
        if ($result[0]['standard_shipping'] == 1) {
            ...
            if ((isset($_POST['pincode_id']) && !empty($_POST['pincode_id'])) or (isset($_POST['pincode']) && !empty($_POST['pincode']))) {
                ...
                if (isset($_POST['pincode_id']) && !empty($_POST['pincode_id'])) {
                    ...
                    $data_with_cod = array('pickup_location' => $pickup_location[0]['pin_code'], 'delivery_pincode' => $pincode[0]['pincode'], 'weight' =>  $result[0]['weight'], 'cod' => 1);
                    $data_without_cod = array('pickup_location' => $pickup_location[0]['pin_code'], 'delivery_pincode' => $pincode[0]['pincode'], 'weight' => $result[0]['weight'], 'cod' => 0);

                    // check shiprocket deliverybility
                    ...
                    if ($shiprocket_data_with_cod['status'] == 200) {
                        ...
                        $response['error'] = true;
                        $response['message'] = $shiprocket_data_with_cod;
                    }
                    print_r(json_encode($response));
                    return false;
                } else {
                    ...
                    $db->sql($sql);
                    $pickup_location = $db->getResult();
                    $data_with_cod = array('pickup_location' => $pickup_location[0]['pin_code'], 'delivery_pincode' => $pincode, 'weight' => $result[0]['weight'], 'cod' => 1);
                    $data_without_cod = array('pickup_location' => $pickup_location[0]['pin_code'], 'delivery_pincode' => $pincode, 'weight' => $result[0]['weight'], 'cod' => 0);
                    $shiprocket_data_with_cod = $shiprocket->check_serviceability($data_with_cod);
                    $shiprocket_data_without_cod = $shiprocket->check_serviceability($data_without_cod);
                    if ($shiprocket_data_with_cod['status'] == 200) {
                        $delivery_places_with_cod = $fn->shiprocket_recomended_data($shiprocket_data_with_cod);
