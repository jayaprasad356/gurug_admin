...
class custom_functions
{
    ...
    public function make_shipping_parcels($data)
    {
        ...
        foreach ($data as $product) {
            for ($i = 0; $i < count($product['item']); $i++) {
                if (
                    $product['item'][$i]['standard_shipping'] &&
                    !empty($product['item'][$i]['pickup_location'])
                ) {
    ...
    {
        $min_days = $max_days = $total_delivery_charge  = 0;
        $shiprocket = new Shiprocket;
        foreach ($data as $pickup_location => $parcels) {
            foreach ($parcels as $seller_id => $parcel) {
                $pin_code = $this->get_data($column = ['pin_code'], "pickup_location='" . $pickup_location . "'", 'pickup_locations');
                $shiprocket_data = array('pickup_location' => $pin_code[0]['pin_code'], 'delivery_pincode' => $user_pincode, 'weight' => (int)$parcel, 'cod' => $is_cod);
                ...
                $data[$pickup_location][$seller_id]['delivery_charge'] = $shiprocket_data['rate'];
                ...
                $total_delivery_charge += $data[$pickup_location][$seller_id]['delivery_charge'];
            }
        }
        $delivery_day = ($min_days == $max_days) ? $min_days : $min_days . '-' . $max_days;
        $shipping_parcels = array('error' => false, 'estimated_delivery_days' => $delivery_day, 'delivery_charge' => round($total_delivery_charge), 'data' => $data);
    ...
    // public function get_shiprocket_wallet_balance()
    // {
    //     $shiprocket = new Shiprocket;
    //     $amount=$shiprocket->get_wallet_balance();
    //     // return $amount;
    //     echo "yes";

    //     print_r($shiprocket->get_wallet_balance());
    // }
}
...
// print_r($fn->get_shiprocket_wallet_balance());
