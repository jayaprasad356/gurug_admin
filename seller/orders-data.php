...
if (isset($res[0]) && !empty($res[0])) {
    foreach ($res as $row) {
        ...
        // $count_standard_product = (isset($row['standard_shipping']) && !empty($row['standard_shipping'])) ? $count_standard_product + $row['standard_shipping'] : $count_standard_product;
                                                            ...
                                                            <div class="card col-md-3">
                                                                <div class="card-body">
                                                                    ...
                                                                    if (!empty($item[14])) {
                                                                        ...
                                                                        $sql = "SELECT `name` FROM `seller` where id= $seller_id ORDER BY id DESC";
                                                                        $db->sql($sql);
                                                                        $seller_name = $db->getResult();
                                            ...
                                            for ($i = 0; $i < count($res); $i++) {
                                                foreach ($items as $item) {
                                                    ...
                                                    if ($res[$i]['standard_shipping']) {
                                                        $sql = 'Select order_item_id,shipment_id,awb_code,is_canceled,courier_company_id,pickup_status from order_trackings where order_item_id=' . $item[10];
                                                        $db->sql($sql);
